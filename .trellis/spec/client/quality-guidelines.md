...
- 纯 C# 计算、协议解析、数据转换和无 Unity 依赖逻辑，优先使用 EditMode 或独立逻辑测试。
- 涉及 MonoBehaviour 生命周期、Prefab、场景、物理、输入、资源加载和时间行为时，使用 PlayMode 或编辑器冒烟验证。
- 验证不能只看进程退出码，还要检查 Unity Console 和实际行为。
- 没有实际运行的测试，不得声称“测试通过”。
- 对象池、异步任务、事件订阅、场景切换和启用/禁用流程，必须验证清理和重复使用。
- 无法运行 Unity 或测试工具时，应明确报告静态检查结果和未验证部分。
- 新测试应按“被测行为 + 预期结果”命名，不要使用 `Test1`、`TestA` 等无意义名称。
- 可行时先定义能复现问题的最小测试，确认修改前能够暴露问题，修改后能够验证修复。
- 测试代码应放在所属功能层附近，并遵循该层已有的测试程序集和命名约定。
