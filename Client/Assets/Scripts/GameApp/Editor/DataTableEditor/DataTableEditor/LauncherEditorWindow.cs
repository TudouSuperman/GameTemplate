...
namespace GameApp.Editor
{
    public class LauncherEditorWindow : EditorWindow
    {
        ...
        private void ButtonNew()
        {
            ...
            m_DataTableEditingWindow.SetData(DataTableEditorUtility.DataTableUtility.NewDataTableFile(m_CurrentEncoding), m_CurrentEncoding);
        }

        private void ButtonLoad()
        {
            var m_DataTableEditingWindow = new DataTableEditingWindowInstance();
            string filePath = EditorUtility.OpenFilePanel("加载数据表格文件", Application.dataPath + "/Res/Config", "txt");
