...
namespace GameApp.Editor
{
    public class BuildToolEditor : EditorWindow
    {
        ...
        [MenuItem("GameApp/Build Tool Editor")]
        ...
        private void OnGUI()
        {
            EditorGUI.BeginDisabledGroup(Application.isPlaying);
            {
                ...
                if (GUILayout.Button("Build Package (Contains Rebuild Resource)"))
                {
                    if (SwitchActiveBuildTarget(m_Platform))
                    {
                        BuildHelper.BuildPkg(m_Platform);
                        Debug.Log($"Build {m_Platform} Package Success!");
                    }
                }
                ...
                GUILayout.Space(20);
                if (GUILayout.Button("Build And Refresh Windows64 Package Resource"))
                {
